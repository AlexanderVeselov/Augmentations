...
class Resize(object):
    ...
    def __call__(self, image, boxes=None):
        ...
        height_scale, width_scale = self.size/o_height, self.size/o_width
        temp_image = cv2.resize(temp_image, (self.size, self.size))
        temp_image = np.array(temp_image, dtype=np.float)
        temp_bboxes = boxes.copy()
        temp_bboxes[:, [0, 2]] *= width_scale
        temp_bboxes[:, [1, 3]] *= height_scale
        temp_bboxes[:, [0, 2]] = np.clip(temp_bboxes[:, [0, 2]], 0, self.size)
        temp_bboxes[:, [1, 3]] = np.clip(temp_bboxes[:, [1, 3]], 0, self.size)
...
class ToCV2Image(object):
    def __call__(self, tensor, boxes=None):
        return tensor.cpu().numpy().astype(np.float32).transpose((1, 2, 0)), boxes
